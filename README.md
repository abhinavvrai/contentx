...
Current production release label:

- `frame-native-20-production-ready-7` (13 September 2026; live)
...
Production verification: all 104 unit/regression tests pass, the clean vinext production build succeeds, and Wrangler 4.92.0 strict dry-run succeeds with the existing D1/R2/assets bindings. GitHub `main` contains application commits `59f5448` and `3b148ec`, Cloudflare serves the `frame-native-20-production-ready-7` release marker, and the signed-in production workspace loads normally. Live review confirmed the dark/light hero treatment, feathered contact illustration, hidden empty favourite marker, centered organizer dialog and the responsive two-column organizer at compact desktop widths. No database migration, secret, binding or payment change was required.
...
Production verification: 103 unit/regression tests pass and the clean production build completes with `/api/admin/coupons` included. Migration `0012_discount_codes.sql` executed 11 statements successfully against production D1 after confirming the new tables/columns were absent. GitHub `main` contains commit `d5d9a4a`; Cloudflare Worker version `09346c84-66d8-46c8-9949-7eb73ecd0aad` serves the release. The apex and `/site/` return 200, `www` returns 308 to the apex, authentication reports Google/email OTP/password reset and D1 available, Razorpay configuration returns 200, and anonymous coupon-admin access returns 403. Browser checks in separate signed-in client and administrator tabs confirmed complete light workspace/account/admin surfaces, dark/light coupon controls, saved theme restoration, checkout coupon input and no console errors. No real charge or test coupon record was created during verification.
