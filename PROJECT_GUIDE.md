...
## 17. Precision Review and Editor Handoff

Current release: `frame-native-14`.

`frame-native-14` adds a persistent per-project In/Out loop for repeatedly checking problem moments, feedback pins positioned over the review timeline, side-by-side or draggable-wipe version comparison, and review-note exports in TXT, spreadsheet-safe CSV and CMX-style EDL. EDL export includes only timecoded notes and rejects empty timecode sets; CSV cells neutralize formula-leading characters. These controls are deliberately concentrated in the review surfaces so the project workspace remains compact. They are original Content X implementations informed by common professional-review workflows, not copied third-party UI.

## 18. Reliable Voice Review and Recoverable Uploads

Current release: `frame-native-15`.

`frame-native-15` replaces the fragile one-button voice flow with a stable recorder that survives asynchronous stop events, closes every microphone track, reports permission/device problems clearly, supports microphone selection and testing, displays a live waveform and timer, and provides pause, resume, finish, cancel, preview, speed, record-again and discard controls. A failed send keeps the recording in memory for retry; if the private R2 voice object already exists, the retry reuses that ID instead of uploading a duplicate. Text comment drafts are stored only in device session storage and never include access credentials.

The real review room also adds next/previous open-note navigation, keyboard seeking and frame stepping, playback speed, mute, Picture-in-Picture, fullscreen and an on-demand shortcut reference. Review comments persist optional start/end ranges, parent replies, priority, assignee, due date, internal-team visibility and open/in-progress/completed workflow states. Shared links can read only project-visible notes; internal notes require account access, and workflow edits require project-manager access. Multipart uploads can pause safely between completed chunks, resume without discarding those chunks, or cancel and invoke the existing server-side multipart cleanup path. The global workspace rail loads unread notification count without delaying the project screen; Mark all read updates every unread account notification in one authenticated, same-origin request. The accepted phased product scope is tracked in `docs/APP_WORKSPACE_ROADMAP.md`; public marketing and conversion work remain outside that scope.

Project managers can remove an asset without immediately destroying its private R2 objects. All ready versions move together into Recently deleted, disappear from normal file/version queries, and can be restored together for 30 days. Shared-link visitors cannot see, remove or restore deleted assets. Permanent deletion remains owned by the existing retention cleanup or the separately protected owner-admin flow; never add a one-click permanent-delete action to the normal project grid.

## 19. Version Decisions and Approval History

Current release: `frame-native-16`.

`frame-native-16` adds explicit Approve version and Request changes decisions to the authenticated review room and private share-link review. Each action appends an immutable D1 record tied to the exact project, asset and file version, with reviewer name, optional email, optional note and time. Approval is rejected while that reviewer can see unresolved feedback on the chosen version; project-internal notes stay invisible to share-link visitors and do not leak through the approval check. Newer open feedback visually supersedes an older approval without rewriting its history. Deleting a whole project also deletes its decision records, while ordinary asset recovery preserves them with the restored version stack.

The same release adds manager-only multi-select for bulk folder moves and recoverable bulk removal, plus Complete/Reopen for the currently filtered feedback set. These bulk writes accept at most 100 validated IDs and re-scope every update to the authorized project and exact file. Video frame capture is browser-local, bounded to 1920 pixels wide and downloaded as PNG; it never uploads a derived frame or exposes the signed media URL.

Workspace route failures now preserve the last usable project screen and show a compact inline Retry banner instead of replacing the whole app. Phones receive a persistent bottom navigation for Home, Projects, Search, Alerts and Account. Each multipart upload part retries only transient network, timeout, rate-limit or server failures, with two short backoff attempts and a pause/cancel checkpoint before every retry; client validation and authorization errors are never retried.

## 20. Secure Share Controls and Project Activity

Current release: `frame-native-17`.

`frame-native-17` makes every private review link independently configurable. Managers can set an exact expiry, optional password, selected-file scope and separate permissions for uploads, original downloads, comments, approval and previous versions. Passwords are PBKDF2-SHA-256 derived with a unique salt and are never stored or returned in readable form. The browser keeps an entered share password in memory only; it never writes it to browser storage or the URL.

Preview signatures are now cryptographically bound to inline viewing or original download intent. A disabled download cannot be recovered by rewriting a preview URL. Selected-file restrictions are rechecked for project listing, version history, comments, decisions, preview/download links and replacement uploads. A scoped upload link can add a replacement only to an included asset; it cannot create an unscoped new asset.
...
Current verified production release: `frame-native-20-production-ready-4`.
