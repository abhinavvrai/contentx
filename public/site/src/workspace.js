...
function renderWorkspaceShell(root, actions, user, projects, selected, projectData, shares, storage, comments, accountPanel = false) {
  ...
  root.innerHTML = `<div class="workspace-shell ${project && !accountPanel ? "project-open" : accountPanel ? "account-open" : "overview-open"}">
    ...
    <aside class="workspace-sidebar">
      ...
      <nav><a class="${accountPanel ? "" : "active"}" href="#workspace"><span>${workspaceIcon("projects")}</span>Projects</a><a class="${accountPanel ? "active" : ""}" href="#workspace?panel=account"><span>${workspaceIcon("account")}</span>Account</a><button type="button" data-create-free-project><span>${workspaceIcon("plus")}</span>New project</button><a class="workspace-website-nav" href="#home" title="View marketing website"><span>🌐</span>Website</a></nav>
  ...
  root.querySelectorAll("[data-open-script-studio]").forEach(button => button.addEventListener("click", () => openScriptStudioModal(root, project, folders, actions)));
