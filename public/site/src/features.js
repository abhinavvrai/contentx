    ...
    try {
      const [configResponse, orderResponse] = await Promise.all([
        ...
        fetch("/api/payments/razorpay/order", {
          ...
          body:JSON.stringify({ planId:razorpayPlanId(plan), quantity:razorpayQuantity(plan), billing:plan.billing || (plan.unit === "month" ? "monthly" : "one_off"), addOns:(plan.addOns || []).map(item => item.id).filter(id => !["longform_extra_minutes", "longform_raw_review"].includes(id)), durationMinutes:plan.durationMinutes, rawFootageHours:plan.rawFootageHours, currency:activeCurrency, contentType:plan.contentType || "video", deliveryFormat:plan.deliveryFormat || "", name:contact.name, email:contact.email, phone:contact.phone })
        })
      ]);
