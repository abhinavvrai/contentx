...
function setupUnifiedPricing(pricing, actions, data) {
  const packages = {
    video: [
      { id:"basic_reel", name:"Basic", price:1500, range:"₹1,500–₹2,000", tag:"BUDGET · UP TO 1 MIN", delivery:"3-day delivery", revisions:"1 revision", summary:"Clean captions-only editing for simple reels that need pacing, subtitles and a publish-ready export.", recommendedAddOns:["broll_sfx"], includes:["1 video up to 60 seconds", "Clean cuts, pacing and zooms", "Engaging captions and subtitles", "Basic colour and audio balance", "1080p social export", "Source file included"], unavailable:["B-roll + sound design is optional +₹500", "Motion graphics", "Advanced motion graphics"] },
      { id:"better_edit", name:"Standard", price:2000, range:"₹2,000–₹2,500", tag:"STANDARD · BETTER EDIT", delivery:"4-day delivery", revisions:"2 revisions", featured:true, summary:"A stronger social edit with cleaner rhythm, better visual layering and an optional motion upgrade.", recommendedAddOns:["motion_graphics"], includes:["1 video up to 90 seconds", "Better edit with sharper pacing", "Up to 5 relevant B-roll inserts", "Music, sound design and timed SFX", "Branded captions and subtitles", "Source file included"], unavailable:["Motion graphics upgrade is optional +₹500", "Advanced motion graphics"] },
      { id:"premium_motion", name:"Premium", price:3500, range:"₹3,500–₹5,000", tag:"PREMIUM · MOTION READY", delivery:"5-day priority", revisions:"3 revisions", summary:"A high-retention premium edit with richer sound, B-roll and motion-ready finishing for flagship posts.", recommendedAddOns:["advanced_motion_graphics"], includes:["1 video up to 180 seconds", "Premium edit with retention-led structure", "Up to 10 relevant B-roll inserts", "Full sound design and premium mix", "Custom callouts and motion titles", "Premium colour finish", "Source file included"], unavailable:["Advanced motion graphics package is optional"] },
    ],
    podcast: [
  ...
  const addOns = {
    video: [
      { id:"broll_sfx", name:"B-roll + Sound Design", price:500, copy:"Best for Basic: add relevant B-roll, music accents and timed sound effects." },
      { id:"motion_graphics", name:"Motion Graphics", price:500, copy:"Best for Standard: animated callouts, titles, icons and branded graphic movement." },
      { id:"advanced_motion_graphics", name:"Advanced Motion Graphics", price:1500, copy:"Best for Premium: custom animated scenes, tracked graphics and premium transitions." },
  ...
  pricing.innerHTML = `<div class="section-heading centered"><p class="eyebrow"><span></span>Simple, flexible pricing</p><h2>Choose the work. Add only what you <em>need.</em></h2><p>Start with Video or Podcast, compare each package, then add B-roll, sound or motion before payment.</p></div><div class="pricing-primary-toggles"><div><small>How often?</small><div class="billing-toggle" role="tablist" aria-label="Billing type"><button type="button" data-unified-billing="one_off" class="active">Per video</button><button type="button" data-unified-billing="monthly">Monthly</button></div></div><div><small>What are we making?</small><div class="billing-toggle service-toggle" role="tablist" aria-label="Content type"><button type="button" data-unified-service="video" class="active">Video</button><button type="button" data-unified-service="podcast">Podcast</button></div></div></div><div class="unified-pricing-builder"><section class="unified-builder-main"><div class="unified-step"><header><span>01</span><div><strong data-package-heading>Choose a video package</strong><small>Switch between the tabs to compare every detail.</small></div></header><div class="unified-package-browser" data-unified-packages></div></div><div class="unified-step"><header><span>02</span><div><strong>Choose quantity and format</strong><small data-quantity-note>Buy one video or build a larger batch.</small></div></header><div class="unified-quantity-row"><label>Quantity <span><button type="button" data-unified-quantity="minus" aria-label="Decrease quantity">−</button><b data-unified-count>1</b><button type="button" data-unified-quantity="plus" aria-label="Increase quantity">+</button></span></label><label>Delivery format<select data-unified-format></select></label></div></div><div class="unified-step" data-addons-step><header><span>03</span><div><strong>Enhance your edit</strong><small>Add B-roll and sound design, motion graphics or advanced motion only when you need them.</small></div></header><div class="unified-addon-grid" data-unified-addons></div></div></section><aside class="unified-summary"><span data-unified-badge>ONE-OFF PER VIDEO</span><p>Your package</p><h3 data-unified-summary-name></h3><small data-unified-summary-meta></small><ul data-unified-summary-list></ul><div class="unified-total-lines"><span>Package <b data-unified-base></b></span><span>Add-ons <b data-unified-addons-total></b></span></div><div class="calculated-total"><small>Total before payment</small><strong data-unified-total></strong><span data-unified-effective></span></div><p class="included-note">You’ll add the title, description, instructions, reference links and files immediately after payment.</p><button class="pill pill-hot" type="button" data-unified-checkout>Continue securely →</button></aside></div><section class="managed-services"><div class="managed-services-head"><p class="eyebrow"><span></span>Need more than editing?</p><h3>Build a complete content system.</h3><p>These managed services are scoped around your brand, publishing volume and goals.</p></div><div class="managed-service-grid">${[
  ...
  function renderPackages() {
    ...
    const recommended = addOns[state.service].filter(item => (plan.recommendedAddOns || []).includes(item.id));
    packageContainer.innerHTML = `<div class="unified-plan-tabs" role="tablist" aria-label="${state.service} packages">${packages[state.service].map(item => `<button type="button" role="tab" aria-selected="${item.id === state.planId}" class="${item.id === state.planId ? "active" : ""}" data-plan-tab="${item.id}">${item.name}<small>${item.range || money(item.price)}</small></button>`).join("")}</div><article class="unified-plan-detail"><p class="unified-plan-label">${plan.tag}</p><div class="unified-plan-price"><strong>${money(plan.price)}</strong><span>starting per ${state.service === "podcast" ? "episode" : "video"}</span></div><p class="unified-plan-copy">${plan.summary}</p><div class="unified-plan-meta"><span>◷ <b>${plan.delivery}</b></span><span>⟳ <b>${plan.revisions}</b></span></div>${recommended.length ? `<div class="unified-plan-upgrades"><strong>Package upgrade option</strong>${recommended.map(item => `<button type="button" data-plan-addon="${item.id}"><span>${item.name}</span><b>+${money(item.price)}</b><small>${money(plan.price + item.price)} total per ${state.service === "podcast" ? "episode" : "video"}</small></button>`).join("")}</div>` : ""}<ul>${plan.includes.map(feature => `<li><b>✓</b>${feature}</li>`).join("")}${(plan.unavailable || []).map(feature => `<li class="unavailable"><b>✓</b>${feature}</li>`).join("")}</ul><button type="button" class="unified-plan-continue" data-plan-continue>Continue with ${plan.name}<span>→</span></button></article>`;
    packageContainer.querySelectorAll("[data-plan-tab]").forEach(button => button.addEventListener("click", () => { state.planId = button.dataset.planTab; renderPackages(); updateSummary(); }));
    packageContainer.querySelectorAll("[data-plan-addon]").forEach(button => button.addEventListener("click", () => {
      const id = button.dataset.planAddon;
      state.selectedAddOns.has(id) ? state.selectedAddOns.delete(id) : state.selectedAddOns.add(id);
      renderPackages(); renderAddOns(); updateSummary();
    }));
    packageContainer.querySelector("[data-plan-continue]").addEventListener("click", () => pricing.querySelector("[data-addons-step]").scrollIntoView({ behavior:"smooth", block:"start" }));
  }
