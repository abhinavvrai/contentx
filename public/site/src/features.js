...
export function initTheme() {
  document.documentElement.dataset.theme = localStorage.getItem("cx_theme") === "light" ? "light" : "dark";
}

export function enhanceMarketing(root, actions, data = {}) {
  const nav = root.querySelector(".site-nav nav");
  if (nav) nav.insertAdjacentHTML("beforeend", '<a href="#contact-form">Contact</a><button class="site-theme-toggle" type="button" data-site-theme aria-label="Switch light or dark mode">◐ Theme</button>');
  root.querySelector("[data-site-theme]")?.addEventListener("click", () => {
...
export async function renderAdmin(root, actions) {
  ...
  root.innerHTML = `<div class="admin-shell"><aside class="owner-sidebar"><div class="owner-workspace"><span class="brand-mark">CX</span><p><strong>Content X</strong><small>Owner workspace</small></p><button type="button" aria-label="Owner menu">•••</button></div><nav aria-label="Owner navigation"><small>WORKSPACE</small><button class="active" data-admin="overview"><span>⌂</span>Overview</button><button data-admin="clients"><span>◉</span>Client workspaces</button><button data-admin="users"><span>◎</span>Website users</button><button data-admin="payments"><span>₹</span>Payments</button><small>OPERATIONS</small><button data-admin="moderation"><span>◷</span>Approval queue</button><button data-admin="managed-review"><span>✦</span>Managed review</button><button data-admin="team"><span>◇</span>Team access</button><button data-admin="applications"><span>↗</span>Talent applications</button><button data-admin="leads"><span>✉</span>Enquiries</button><small>SETTINGS</small><button data-admin="settings"><span>⚙</span>Review controls</button></nav><div class="owner-sidebar-actions"><button data-client-view>← Client workspace</button></div></aside><main><header><div><p>Owner workspace</p><h1>Operations overview</h1></div><button class="pill pill-hot" data-add-client>+ Add offline-paid client</button></header><section class="admin-content"></section></main></div>`;
  root.querySelector(".owner-sidebar-actions")?.insertAdjacentHTML("beforeend", '<button data-owner-theme>◐ Light / dark</button><button data-owner-lock>⌾ Lock owner session</button>');
  const content = root.querySelector(".admin-content");
  let adminDirectory = { projects:[], projectAccess:[], recentUploads:[], recentActivity:[] };
  ...
  const teamPermissionsView = () => {
    ...
    return `<div class="dash-section-head"><div><h2>Team roles & permissions</h2><p>Give each team member only the clients, projects and actions they need.</p></div><button class="pill pill-hot">+ Invite teammate</button></div><section class="team-permission-board"><aside><span>Owner lock</span><h3>You keep master control.</h3><p>Owner can view every client, change roles, restrict downloads/comments and remove access anytime.</p><div class="permission-chip-row"><b>All clients</b><b>All payments</b><b>All settings</b></div></aside><div class="permission-grid">${permissions.map(([title,copy])=>`<article><strong>${title}</strong><small>${copy}</small><label><span>Allowed</span><input type="checkbox" checked></label></article>`).join("")}</div></section><div class="team-grid team-grid-permissions">${people.map(([initials,name,role,scope,access])=>`<article><span>${initials}</span><h3>${name}</h3><p>${role}</p><small><b>Scope:</b> ${scope}</small><small><b>Access:</b> ${access}</small><button>Manage permissions →</button></article>`).join("")}</div>`;
  };
