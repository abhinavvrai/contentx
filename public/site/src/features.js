...
export async function renderAdmin(root, actions) {
  ...
  let adminDirectory = { projects:[], projectAccess:[] };
  ...
  const usersView = async () => {
    ...
    try {
      ...
      adminDirectory = { projects:payload.projects || [], projectAccess:payload.projectAccess || [] };
  ...
  content.addEventListener("click", async event => {
    ...
    try {
      ...
      if (button.dataset.userAction === "delete") {
        if (!confirm("Schedule this account for deletion in 30 days? Access stops immediately, while payment and project records remain recoverable during the safety period.")) return;
        await runAdminUserAction(userId, "set_user_status", { status:"deletion_pending" });
