...
export function renderMarketing(root, data, actions) {
  ...
  root.innerHTML = `
    ...
    <main id="top">
      ...
      <section id="workflow" class="workflow-section block-section"><div class="section-shell"><div class="section-heading split"><div><p class="eyebrow"><span></span>Your workflow</p><h2>From raw footage to <em>approved.</em></h2></div><p>Everything your project needs lives in one place—so feedback stays clear and delivery keeps moving.</p></div><div class="workflow-grid">${data.workflow.map(w => `<article><span>${w.step}</span><div class="step-icon">${["↑","✦","◌","✓"][Number(w.step)-1]}</div><h3>${w.title}</h3><p>${w.copy}</p></article>`).join("")}</div><div class="feature-banner"><div><span class="live-dot"></span><small>THE CONTENT X WORKSPACE</small><h3>Review video without the back-and-forth.</h3><p>Click any moment to add a timestamped note. Compare versions, resolve feedback and approve the final cut—all in your browser.</p><button class="pill pill-light" data-action="workspace">Open interactive demo →</button></div><div class="review-mini"><div class="review-video"><video src="videos/video3.mp4" muted loop playsinline autoplay preload="auto" data-preview-autoplay></video><span>00:12</span></div><div class="review-note"><b>MK</b><p><strong>00:12</strong> Can we make this transition faster?</p><button>Reply</button></div></div></div></div></section>
...
export function renderReview(root, actions) {
  ...
  const renderComments = () => {
    ...
    root.querySelectorAll("[data-delete-comment]").forEach(btn => btn.addEventListener("click", () => { const c = comments.find(x => x.id === Number(btn.dataset.deleteComment)); if (!c || !canManageComment(c)) return; if (!confirm("Delete this comment?")) return; comments = comments.filter(item => item.id !== c.id); save(); renderComments(); root.querySelector(".comment-head h2 span").textContent = comments.length; }));
  };
