...
export function renderMarketing(root, data, actions) {
  ...
  root.innerHTML = `
    <header class="site-nav">
      ...
      <div class="nav-actions"><button class="text-button" data-action="login">Client login</button><a class="pill pill-hot" href="${data.whatsapp}" target="_blank" rel="noreferrer">Start a project <span>↗</span></a></div>
    </header>
    <main id="top">
      <section class="hero section-shell">
        ...
        <div class="hero-content">
          ...
          <div class="hero-actions"><a class="pill pill-hot pill-large" href="${data.whatsapp}" target="_blank" rel="noreferrer">${data.hero.primary} ↗</a><button class="pill pill-dark pill-large" data-action="workspace">${data.hero.secondary} →</button></div>
      ...
      <section id="workflow" class="workflow-section block-section"><div class="section-shell"><div class="section-heading split"><div><p class="eyebrow"><span></span>Your workflow</p><h2>From raw footage to <em>approved.</em></h2></div><p>Everything your project needs lives in one place—so feedback stays clear and delivery keeps moving.</p></div><div class="workflow-grid">${data.workflow.map(w => `<article><span>${w.step}</span><div class="step-icon">${["↑","✦","◌","✓"][Number(w.step)-1]}</div><h3>${w.title}</h3><p>${w.copy}</p></article>`).join("")}</div><div class="feature-banner"><div><span class="live-dot"></span><small>THE CONTENT X WORKSPACE</small><h3>Review video without the back-and-forth.</h3><p>Click any moment to add a timestamped note. Compare versions, resolve feedback and approve the final cut—all in your browser.</p><button class="pill pill-light" data-action="workspace">Open interactive demo →</button></div><div class="review-mini"><div class="review-video"><video src="videos/premium2.mp4" muted loop playsinline autoplay></video><span>00:12</span></div><div class="review-note"><b>MK</b><p><strong>00:12</strong> Can we make this transition faster?</p><button>Reply</button></div></div></div></div></section>
      <section id="pricing" class="section-shell block-section"><div class="section-heading centered"><p class="eyebrow"><span></span>Simple pricing</p><h2>Choose the level of <em>polish.</em></h2><p>Two revision rounds are included in every package. Additional revisions are ₹300 per round.</p></div><div class="pricing-grid">${data.plans.map(plan => `<article class="price-card ${plan.featured ? "featured" : ""}">${plan.featured ? '<span class="popular">MOST POPULAR</span>' : ""}<h3>${plan.title}</h3><p>${plan.description}</p><strong>${plan.price}</strong><small>${plan.note}</small><ul>${plan.features.map(f => `<li><span>✓</span>${f}</li>`).join("")}</ul><a class="pill ${plan.featured ? "pill-hot" : "pill-dark"}" href="${data.whatsapp}" target="_blank" rel="noreferrer">Choose ${plan.title} →</a></article>`).join("")}</div><p class="pricing-note">Need 8+ videos every month? <a href="${data.whatsapp}" target="_blank" rel="noreferrer">Ask about a custom monthly retainer →</a></p></section>
      <section id="faq" class="section-shell faq-section block-section"><div><p class="eyebrow"><span></span>Questions</p><h2>Before we<br><em>get started.</em></h2><p>Still unsure? Message us and we’ll recommend the right package for your content.</p><a class="pill pill-dark" href="${data.whatsapp}" target="_blank" rel="noreferrer">Ask on WhatsApp ↗</a></div><div class="faq-list">${data.faqs.map((f,i) => `<details ${i===0?"open":""}><summary>${f[0]}<span>+</span></summary><p>${f[1]}</p></details>`).join("")}</div></section>
      <section class="cta-section"><div class="section-shell"><p class="eyebrow light"><span></span>Ready when you are</p><h2>Let’s make your next video <em>impossible to skip.</em></h2><p>Tell us what you’re creating. We’ll reply with the right package, timeline and next steps.</p><div><a class="pill pill-light pill-large" href="${data.whatsapp}" target="_blank" rel="noreferrer">Start on WhatsApp ↗</a><a class="pill pill-outline pill-large" href="mailto:${data.email}">Email ${data.email}</a></div></div></section>
    </main>
...
export function renderReview(root, actions) {
  ...
  root.querySelector(".comment-form").addEventListener("submit", e => { e.preventDefault(); const field = e.currentTarget.querySelector("textarea"); if (!field.value.trim()) return; comments.push({ id: Date.now(), author: "Meera", initials: "MK", time: video.currentTime, text: field.value.trim(), age: "Just now", resolved: false }); field.value = ""; save(); renderComments(); root.querySelector(".comment-head h2 span").textContent = comments.length; });
