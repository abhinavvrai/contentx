...
function startMutedPreviewVideos(root) {
  ...
  const deferred = videos.filter(video => video !== priority);
  if (typeof IntersectionObserver === "undefined") deferred.forEach(start);
  else {
    ...
    root._previewObserver = observer;
  }
}
...
function serviceCard(id, title, price, copy, featured = false) {
  const usdPrice = Math.max(15, Math.round(price / 96));
  return `<article class="service-card ${featured ? "featured" : ""}"><div><h3>${title}</h3><p>${copy}</p></div><strong>$${usdPrice.toLocaleString("en-US")}</strong><small>per project</small><button class="pill ${featured ? "pill-hot" : "pill-dark"}" data-service-plan="${id}">Pay securely →</button></article>`;
}
...
export function renderMarketing(root, data, actions) {
  ...
  if (pricingFallback) pricingFallback.innerHTML = `<div class="section-heading centered"><p class="eyebrow"><span></span>Simple pricing</p><h2>High-retention video editing. <em>Zero overhead.</em></h2><p>Packages start from $100 for 4 videos. The interactive package builder is loading now.</p></div>`;
...
const cxIcon = (name, label = "") => {
  const paths = {
    ...
    annotate:'<path d="m4 20 4.5-1 10-10a2.1 2.1 0 0 0-3-3l-10 10zM14 7l3 3"/>',
...
function dashboardShell(content, active = "projects") {
  return `<div class="dashboard-shell"><aside class="cx-app-rail" aria-label="Global navigation"><a class="cx-rail-brand dash-brand" href="#" aria-label="Content X home">CX</a><nav><button class="${active === "home" ? "active" : ""}" aria-label="Home">${cxIcon("home")}</button><button class="${active === "projects" ? "active" : ""}" aria-label="Projects">${cxIcon("projects")}</button><button class="${active === "scripts" ? "active" : ""}" aria-label="Scripts" data-dash="scripts" title="Script Writing Studio">${cxIcon("script")}</button><button aria-label="Review">${cxIcon("review")}</button><button aria-label="Search">${cxIcon("search")}</button></nav><div class="cx-rail-bottom"><button aria-label="Settings" title="Settings">${cxIcon("settings")}</button><button class="cx-rail-avatar" aria-label="Account">MK</button></div></aside><aside class="dash-sidebar"><div class="dash-workspace"><strong>Content X</strong><small>Apex Fitness</small><button aria-label="Workspace menu">•••</button></div><nav><small>WORKSPACE</small><button class="${active === "home" ? "active" : ""}" data-dash="home">${cxIcon("home","Home")}</button><button class="${active === "projects" ? "active" : ""}" data-dash="projects">${cxIcon("projects","Projects")}<b>3</b></button><button class="${active === "scripts" ? "active" : ""}" data-dash="scripts" title="Script Writing Studio">${cxIcon("script","Scripts")}</button><button data-dash="reviews">${cxIcon("review","Needs review")}<b>2</b></button><small>LIBRARY</small><button data-dash="approved">${cxIcon("grid","All assets")}</button><button data-dash="assets">${cxIcon("share","Share links")}</button></nav><div class="storage"><div><span>Storage</span><small>6.4 / 20 GB</small></div><i><em></em></i><button>Manage plan</button></div></aside><main class="dash-main"><div class="cx-product-topbar"><label>${cxIcon("search")}<input type="search" placeholder="Search Content X" aria-label="Search Content X"></label><button aria-label="Notifications">${cxIcon("bell")}</button><button aria-label="Help">${cxIcon("help")}</button></div>${content}</main></div>`;
}
...
function bindDashboard(root, actions) {
  ...
  bindProductNavigation(root, actions);
  ...
  root.querySelectorAll('[data-dash="scripts"]').forEach(b => b.addEventListener("click", () => { location.hash = "workspace?panel=scripts"; }));
}

function bindProductNavigation(root, actions) {
  root.querySelectorAll(".dash-brand,.review-global-rail .cx-rail-brand").forEach(brand => brand.addEventListener("click", event => { event.preventDefault(); actions.openMarketing(); }));
  root.querySelectorAll('.cx-app-rail button[aria-label="Home"],.review-global-rail button[aria-label="Home"]').forEach(button => button.addEventListener("click", actions.openDashboard));
  root.querySelectorAll('.cx-app-rail button[aria-label="Projects"],.review-global-rail button[aria-label="Projects"]').forEach(button => button.addEventListener("click", actions.openDashboard));
  root.querySelectorAll('.cx-app-rail button[aria-label="Review"],.review-global-rail button[aria-label="Review"]').forEach(button => button.addEventListener("click", actions.openReview));
  root.querySelectorAll('.cx-app-rail button[aria-label="Search"],.review-global-rail button[aria-label="Search"]').forEach(button => button.addEventListener("click", () => {
    const field = root.querySelector('.cx-product-topbar input[type="search"],.project-toolbar input,.review-search input');
    field?.focus(); field?.select?.();
  }));
  root.querySelectorAll('.cx-app-rail button[aria-label="Account"],.review-global-rail button[aria-label="Account"]').forEach(button => button.addEventListener("click", actions.openAccess));
}
...
export function renderReview(root, actions) {
  ...
  const currentUser = JSON.parse(localStorage.getItem("cx_comment_user") || "null") || { name: "Meera", initials: "MK", role: "client" };
    root.dataset.reviewMobileMode = "player";
    root.innerHTML = `<aside class="review-global-rail" aria-label="Global navigation"><button class="cx-rail-brand" aria-label="Content X">CX</button><nav><button aria-label="Home">${cxIcon("home")}</button><button class="active" aria-label="Projects">${cxIcon("projects")}</button><button aria-label="Review">${cxIcon("review")}</button><button aria-label="Search">${cxIcon("search")}</button></nav><button class="cx-rail-avatar" aria-label="Account">MK</button></aside>
    <header class="review-header"><button class="back-button" data-action="back" aria-label="Back to project">${cxIcon("back")}</button><div class="review-breadcrumb"><strong>Apex Fitness Launch</strong><span>/</span><small>Launch Reel 01</small></div><label class="review-search">${cxIcon("search")}<input type="search" placeholder="Search feedback" aria-label="Search feedback"></label><span class="review-position">2 of 5</span><span class="version-select">Version 3⌄</span><div class="review-head-actions"><button class="pill pill-dark">${cxIcon("share")} Share</button><button class="pill pill-dark" data-action="changes">Request changes</button><button class="pill pill-green" data-action="approve">${cxIcon("check")} Approve</button></div></header>
    <nav class="review-mobile-nav" role="tablist" aria-label="Review view"><button type="button" class="active" data-route-mobile-mode="player" role="tab" aria-selected="true">🎬 Video</button><button type="button" data-route-mobile-mode="notes" role="tab" aria-selected="false">💬 Feedback (${comments.length})</button><button type="button" data-route-mobile-mode="split" role="tab" aria-selected="false">◫ Split</button></nav>
    <main class="review-layout"><section class="review-stage"><div class="stage-status"><span class="status in-review"><i></i>Waiting for review</span><span>Click the video or press Space to pause and resume</span></div><div class="player-wrap" tabindex="0" aria-label="Video player. Click to pause or resume"><video src="videos/premium1.mp4" muted autoplay playsinline preload="metadata" controlsList="nodownload noplaybackrate noremoteplayback"></video><span class="playback-flash" aria-hidden="true"></span></div><div class="player-controls"><button class="player-play-toggle" data-action="play" aria-label="Pause video">${cxIcon("pause")}<small>Pause</small></button><span class="current-time">00:00</span><input type="range" min="0" max="100" value="0" step="0.1" aria-label="Video timeline"><span class="duration">00:32</span><button aria-label="Playback settings">${cxIcon("settings")}</button><button data-action="fullscreen" aria-label="Full screen">${cxIcon("fullscreen")}</button></div><div class="version-bar"><div><strong>Versions</strong><small>Compare every cut</small></div><button><span>V1</span><small>Jul 29</small></button><button><span>V2</span><small>Aug 1</small></button><button class="active"><span>V3</span><small>Current</small></button><button class="upload-version">${cxIcon("upload")} New version</button></div></section>
    <aside class="comment-panel"><div class="comment-tabs"><button class="active" data-review-panel="comments">Comments <span>${comments.length}</span></button><button data-review-panel="details">Details</button></div><section class="review-comments-view"><div class="comment-filter"><button class="active">Open</button><button>Resolved</button><span></span><button>Newest⌄</button></div><div class="comments"></div><form class="comment-form"><div class="comment-input-row"><b>${currentUser.initials}</b><textarea rows="3" placeholder="Leave feedback at 00:00…"></textarea></div><p class="comment-attachment-preview" data-attachment-preview hidden></p><div class="comment-action-row"><button type="button" data-attach-comment-media>Attach</button><span>At <strong>00:00</strong></span><button class="send-comment" type="submit">Send</button></div><input type="file" accept="image/*,video/*,audio/*,.pdf,.txt,.md,.csv,.doc,.docx" data-comment-media hidden></form></section><section class="review-details-view" hidden><div><small>ASSET</small><h2>Launch Reel 01</h2><p>Vertical social campaign edit</p></div><dl><div><dt>Format</dt><dd>1080 × 1920</dd></div><div><dt>Duration</dt><dd>00:32</dd></div><div><dt>Frame rate</dt><dd>24 fps</dd></div><div><dt>Version</dt><dd>V3</dd></div><div><dt>Uploaded</dt><dd>12 min ago</dd></div><div><dt>Access</dt><dd>Private</dd></div></dl><button class="pill pill-dark">Download original</button></section></aside></main><div class="toast" role="status"></div>`;
    const routeMobileButtons = root.querySelectorAll("[data-route-mobile-mode]");
    routeMobileButtons.forEach(btn => btn.addEventListener("click", () => {
      const mode = btn.dataset.routeMobileMode;
      root.dataset.reviewMobileMode = mode;
      routeMobileButtons.forEach(b => {
        const active = b === btn;
  ...
  root.querySelector(".review-search input")?.addEventListener("input", event => { const query = event.currentTarget.value.trim().toLowerCase(); root.querySelectorAll(".comment").forEach(comment => { comment.hidden = Boolean(query) && !comment.textContent.toLowerCase().includes(query); }); });
  ...
  root.querySelector(".player-wrap").addEventListener("click", event => { if (!event.target.closest("button,input,a")) togglePlayback(); });
  const setPlayState = playing => {
    playButtons.forEach(button => { button.innerHTML = `${cxIcon(playing ? "pause" : "play")}<small>${playing ? "Pause" : "Play"}</small>`; button.setAttribute("aria-label", playing ? "Pause video" : "Play video"); });
    const flash = root.querySelector(".playback-flash"); if (flash) { flash.innerHTML = cxIcon(playing ? "play" : "pause"); flash.classList.remove("show"); requestAnimationFrame(() => flash.classList.add("show")); }
  };
  ...
  root.querySelector("[data-comment-media]").addEventListener("change", event => { const file = event.target.files?.[0]; if (!file) return; const family = file.type.split("/")[0]; selectedAttachment = { name:file.name, type:family === "video" ? "Video reference" : family === "audio" ? "Audio reference" : family === "image" ? "Image reference" : "Document reference", quality:"Original-quality upload allowed" }; const preview = root.querySelector("[data-attachment-preview]"); preview.hidden = false; preview.textContent = `${selectedAttachment.type}: ${selectedAttachment.name}`; });
  root.querySelector(".comment-form").addEventListener("submit", e => { e.preventDefault(); const field = e.currentTarget.querySelector("textarea"), voice = e.currentTarget.cxVoiceNote; if (!field.value.trim() && !selectedAttachment && !voice) return; const feedback = field.value.trim() || (voice ? "Voice feedback" : "Attached a visual reference."); comments.push({ id: Date.now(), author: currentUser.name, initials: currentUser.initials, time: video.currentTime, text: feedback, voice, attachment:selectedAttachment, age: "Just now", resolved: false, completed: false, edited: false }); postNotificationEvent("comment", "New review comment", `${feedback} · ${format(video.currentTime)}`, { actorName:currentUser.name }); field.value = ""; e.currentTarget.cxVoiceNote = null; selectedAttachment = null; root.querySelector("[data-attachment-preview]").hidden = true; root.querySelector("[data-comment-media]").value = ""; save(); renderComments(); root.querySelector('[data-review-panel="comments"] span').textContent = comments.length; root.dispatchEvent(new CustomEvent("cx:comment-sent")); });
  ...
  root.querySelector('[data-action="fullscreen"]')?.addEventListener("click", () => root.querySelector(".player-wrap")?.requestFullscreen?.());
  root.querySelector('[data-action="back"]').addEventListener("click", actions.openProject);
  const onKey = event => { if (event.code === "Space" && !event.target.closest?.("input,textarea,select,button")) { event.preventDefault(); togglePlayback(); } };
