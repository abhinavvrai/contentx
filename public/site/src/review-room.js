...
export async function openReviewRoom({ layer, api, headers, projectId, assetId, canManage = false, onChange = () => {}, initialCommentId = "" }) {
  ...
  dialog.innerHTML = '<header class="sx-room-header"><h2>Review</h2><button type="button" data-room-close aria-label="Close review">×</button></header><p class="sx-room-loading" role="status">Loading…</p>';
  ...
  try {
    ...
    dialog.dataset.mobileMode = "player";
    dialog.innerHTML = `<header class="sx-room-header"><div><h2>${esc(selected.original_name)}</h2></div><button type="button" data-room-close aria-label="Close review">×</button></header><nav class="sx-mobile-mode-switch" role="tablist" aria-label="Review view mode"><button type="button" class="is-active" data-mobile-mode-tab="player" role="tab" aria-selected="true">🎬 Video</button><button type="button" data-mobile-mode-tab="notes" role="tab" aria-selected="false">💬 Notes (<span data-mobile-notes-count>0</span>)</button><button type="button" data-mobile-mode-tab="split" role="tab" aria-selected="false">◫ Split</button></nav><div class="sx-room-body"><section class="sx-room-stage"><div class="sx-room-toolbar"><label class="sx-toolbar-version"><select data-review-version aria-label="Select version">${versions.map(version => `<option value="${esc(version.id)}">V${Number(version.version_number)} · ${esc(version.original_name)}</option>`).join("")}</select></label><button type="button" data-compare aria-pressed="false" ${versions.length < 2 ? "disabled title=\"Upload another version to compare\"" : ""}>Compare</button><button type="button" data-version-download>Download</button></div><div class="sx-compare-choice" hidden><label>Compare against <select data-compare-version></select></label></div><div class="sx-media-grid"><figure><figcaption data-primary-caption></figcaption><div data-primary-media class="sx-media-slot" tabindex="0"></div></figure><figure data-comparison hidden><figcaption data-compare-caption></figcaption><div data-compare-media class="sx-media-slot"></div></figure></div><div class="sx-mobile-quick-actions"><button type="button" data-mobile-goto-notes>💬 View notes (<span data-mobile-notes-count-action>0</span>)</button><button type="button" data-mobile-quick-note>＋ Add note at playhead</button></div><div class="sx-document-tools" data-document-tools hidden><label>PDF page <input type="number" min="1" max="9999" value="1" data-document-page></label><button type="button" data-quote-selection>Quote selected text</button></div><p data-room-error role="alert" hidden></p><section class="sx-version-strip" aria-label="Version history">${versions.map(version => `<button type="button" data-pick-version="${esc(version.id)}"><span>V${Number(version.version_number)}</span><small>${Number(version.version_number) === Number(versions[0].version_number) ? "Latest cut" : "Earlier cut"}</small></button>`).join("")}</section><div class="sx-review-progress"><span data-feedback-progress></span><progress max="100" value="0" aria-label="Feedback completion"></progress></div></section><aside class="sx-room-feedback"><header><h3>Feedback <span data-note-count></span></h3><div class="sx-export-notes"><select data-export-format aria-label="Export format"><option value="text">TXT</option><option value="csv">CSV</option><option value="edl">EDL</option></select><button type="button" data-export-notes>Export</button></div></header><div class="sx-note-filters"><input type="search" data-note-search aria-label="Search review comments" placeholder="Search feedback…"><select data-note-status aria-label="Filter review comments"><option value="all">All notes</option><option value="open">Open</option><option value="complete">Completed</option></select><select data-note-sort aria-label="Sort review comments"><option value="time">Timecode</option><option value="newest">Newest</option></select></div><div class="sx-room-notes" data-room-notes></div><form class="sx-note-form"><div class="sx-form-author-row"><label>Your name<input name="authorName" required minlength="2" maxlength="100" autocomplete="name" placeholder="Your name"></label><label>Email (optional)<input name="authorEmail" type="email" maxlength="254" autocomplete="email" placeholder="you@company.com"></label></div><label class="sx-form-feedback-label"><textarea name="body" required maxlength="2000" rows="3" placeholder="Add feedback…"></textarea></label><div class="sx-capture-row"><label><input type="checkbox" data-attach-time checked>At <output data-pinned-time>00:00</output></label><button type="button" data-capture-time>Use playhead</button></div><button class="workspace-button primary" type="submit">Send feedback ↗</button><p role="alert" hidden></p></form></aside></div>`;
    ...
    dialog.querySelector(".sx-version-strip").insertAdjacentHTML("afterend", `<section class="sx-version-decision"><header><div><strong data-decision-status>Not decided</strong></div><span data-decision-by hidden></span></header><input type="text" data-decision-note maxlength="500" placeholder="Decision note (optional)"><div class="sx-decision-actions"><button type="button" data-version-decision="changes_requested">Request changes</button><button type="button" data-version-decision="approved">Approve version</button></div><details><summary>Decision history</summary><div data-decision-history></div></details></section>`);
    ...
    if (!canComment) { form.hidden = true; form.insertAdjacentHTML("beforebegin", `<div class="sx-review-locked"><span>◇</span><div><b>Feedback is view-only</b></div></div>`); }
    if (!canApprove) { dialog.querySelectorAll("[data-version-decision]").forEach(button => { button.disabled = true; }); dialog.querySelector(".sx-version-decision").classList.add("is-read-only"); dialog.querySelector(".sx-version-decision .sx-decision-actions").insertAdjacentHTML("beforebegin", `<p class="sx-decision-locked">Approval disabled for this link.</p>`); }
    form.elements.body.required = false;
    form.querySelector(".sx-capture-row").insertAdjacentHTML("beforebegin", `<div class="sx-reply-target" data-reply-target hidden><span>Replying in thread</span><button type="button" data-cancel-reply>Cancel</button><input type="hidden" name="parentCommentId"></div>${canManage ? `<details class="sx-meta-compose-drawer"><summary>Task details (optional)</summary><div class="sx-comment-meta-compose"><label>Priority<select name="priority"><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option><option value="low">Low</option></select></label><label>Assignee<input name="assignee" maxlength="100" placeholder="Name or role"></label><label>Due date<input name="dueDate" type="date"></label><label>Visibility<select name="visibility"><option value="project">Everyone with access</option><option value="internal">Internal team only</option></select></label></div></details>` : ""}`);
    ...
    const setVoiceStatus = (message, state = "idle") => { voiceStatus.textContent = message; voiceStatus.hidden = !message; voiceComposer.dataset.state = state; };
    ...
    const mobileModeButtons = dialog.querySelectorAll("[data-mobile-mode-tab]");
    const setMobileMode = mode => {
      dialog.dataset.mobileMode = mode;
      mobileModeButtons.forEach(btn => {
        const active = btn.dataset.mobileModeTab === mode;
        btn.classList.toggle("is-active", active);
        btn.setAttribute("aria-selected", String(active));
      });
    };
    mobileModeButtons.forEach(btn => btn.addEventListener("click", () => setMobileMode(btn.dataset.mobileModeTab)));
    dialog.querySelector("[data-mobile-goto-notes]")?.addEventListener("click", () => setMobileMode("notes"));
    dialog.querySelector("[data-mobile-quick-note]")?.addEventListener("click", () => {
      capture();
      setMobileMode("notes");
      setTimeout(() => form.elements.body?.focus(), 80);
    });
    const renderNotes = () => {
      ...
      const mobileCount = dialog.querySelector("[data-mobile-notes-count]");
      if (mobileCount) mobileCount.textContent = versionComments.length;
      const mobileActionCount = dialog.querySelector("[data-mobile-notes-count-action]");
      if (mobileActionCount) mobileActionCount.textContent = versionComments.length;
    ...
    const renderDecision = () => {
      ...
      byline.textContent = latest ? `${latest.actor_name || "Reviewer"} · ${new Date(Number(latest.created_at)).toLocaleString()}${newerFeedback ? " · newer feedback added" : ""}` : "";
      byline.hidden = !latest;
      const approve = dialog.querySelector('[data-version-decision="approved"]');
      approve.disabled = !canApprove || openCount > 0;
      approve.title = !canApprove ? "Approval disabled for this link" : openCount ? `Complete ${openCount} open note${openCount === 1 ? "" : "s"} before approval` : "Approve version";
      historyList.innerHTML = versionDecisions.length ? versionDecisions.map(item => `<article><span class="${item.decision === "approved" ? "approved" : "changes"}">${item.decision === "approved" ? "Approved" : "Changes requested"}</span><div><b>${esc(item.actor_name || "Reviewer")}</b><small>${esc(new Date(Number(item.created_at)).toLocaleString())}</small>${item.note ? `<p>${esc(item.note)}</p>` : ""}</div></article>`).join("") : `<p class="sx-note-empty">No decisions yet</p>`;
    };
    ...
    const mountMedia = async (slot, version, secondary, ticket, retry = 0) => {
      ...
      if (!/^(video|audio|image)\//.test(type) && !isPdf && !isText) { slot.textContent = "Preview not supported. Download to view."; return; }
    ...
    dialog.querySelector("[data-room-notes]").addEventListener("click", async event => {
      ...
      if (seek) {
        ...
        if (media && Number.isFinite(media.duration)) {
          media.pause();
          media.currentTime = Math.min(Number(seek.dataset.seek), media.duration);
          capture();
          if (window.innerWidth <= 768 && dialog.dataset.mobileMode === "notes") {
