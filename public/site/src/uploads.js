...
async function renderOwnerProjectFiles(content, projectId) {
  ...
  try {
    const data = await apiRequest(`${API_PATH}?action=admin-files&projectId=${encodeURIComponent(projectId)}`, { headers:ownerHeaders(false) });
    const project = data.project;
    content.innerHTML = `<button class="owner-back" type="button" data-owner-files-back>← All upload projects</button><section class="owner-project-head"><div><p class="eyebrow"><span></span>${project.status === "active" ? "Accepting uploads" : "Uploads paused"}</p><h2>${escapeHTML(project.name)}</h2><p>${escapeHTML(project.clientName || project.clientEmail || "Private client upload project")}</p></div><div><button class="pill pill-dark" type="button" data-rotate-owner-project="${escapeHTML(project.id)}">Create new link</button><button class="pill ${project.status === "active" ? "pill-dark" : "pill-hot"}" type="button" data-project-status="${project.status === "active" ? "archived" : "active"}">${project.status === "active" ? "Pause uploads" : "Resume uploads"}</button></div></section><div class="owner-file-table"><div class="owner-file-row head"><span>File</span><span>Uploaded by</span><span>Size</span><span>Status</span><span></span></div>${data.files.length ? data.files.map(file => `<article class="owner-file-row"><div><b>${fileIcon(file.content_type)}</b><span><strong>${escapeHTML(file.original_name)}</strong><small>${formatDate(file.completed_at || file.created_at)}</small></span></div><span>${escapeHTML(file.uploader_name || file.uploader_email || "Client")}</span><span>${formatBytes(file.size_bytes)}</span><em class="file-status ${escapeHTML(file.status)}">${escapeHTML(file.status)}</em><div><button type="button" data-download-owner-file="${escapeHTML(file.id)}" ${file.status !== "ready" ? "disabled" : ""}>Download</button><button type="button" data-delete-owner-file="${escapeHTML(file.id)}">Delete</button></div></article>`).join("") : `<div class="upload-empty"><span>↑</span><h3>No files received yet</h3><p>Send the project link to your client to begin.</p></div>`}</div>`;
