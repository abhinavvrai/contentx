...
export function renderTalentProfile(root, actions) {
  ...
  const showPackage = index => {
    ...
    panel.innerHTML = `<p>${escapeHTML(item.description)}</p><div><strong>${money(item.price)}</strong><span>Delivery: ${escapeHTML(item.delivery)}</span></div><ul>${item.features.map(feature => `<li><b>✓</b>${escapeHTML(feature)}</li>`).join("")}</ul><button class="pill pill-hot" data-order-package>Choose ${escapeHTML(item.name)} →</button><small>Revision rounds follow the selected package.</small>`;
...
export function enhanceMarketplaceMarketing(root, actions) {
  ...
  if (existing) existing.outerHTML = `<section id="network" class="market-teaser client-service-focus block-section"><div class="section-shell"><div class="section-heading split"><div><p class="eyebrow"><span></span>Managed creative services</p><h2>Build the right team.<br><em>Keep one accountable partner.</em></h2></div><p>Tell Content X what you need. We select the right specialists, manage the work and keep every file, review and approval in one private workspace.</p></div><div class="market-teaser-grid"><article><span>YOUR CONTENT TEAM</span><h3>Specialists selected around your outcome.</h3><p>Get video editing, scripting, strategy, social management, thumbnails and original content ideas without coordinating multiple freelancers.</p><div>${["Video editors", "Scriptwriters", "Social managers", "Strategists", "Thumbnail designers", "Idea creators"].map(item => `<small>✓ ${item}</small>`).join("")}</div><button class="pill pill-hot" data-teaser-hire>Explore managed services →</button></article><article><span>ONE PRIVATE WORKFLOW</span><h3>Brief, review and approve in one place.</h3><p>Content X manages specialist selection and quality while you keep one clear scope, one point of contact and one protected delivery room.</p><ul><li>Private project brief</li><li>Content X quality review</li><li>Versioned delivery and approval</li></ul><button class="pill pill-hot" data-teaser-brief>Submit a project brief →</button></article></div><div class="market-teaser-foot"><span>Brief</span><i>→</i><span>Content X plan</span><i>→</i><span>Production</span><i>→</i><span>Review</span><i>→</i><span>Approve</span></div></div></section>`;
  root.querySelector("[data-teaser-hire]")?.addEventListener("click", actions.openMarketplace);
  root.querySelector("[data-teaser-brief]")?.addEventListener("click", actions.openMarketplace);
  const footerConnect = root.querySelector(".site-footer .section-shell>div:last-child");
  if (footerConnect && !footerConnect.querySelector("[data-join-network]")) {
    footerConnect.querySelector('[data-action="login"]')?.insertAdjacentHTML("beforebegin", '<button data-join-network>Join the creator network</button>');
    footerConnect.querySelector("[data-join-network]")?.addEventListener("click", actions.openProviderOnboarding);
  }
}
...
export function enhanceMarketplaceDashboard(root, actions) {
  ...
  nav?.querySelector('[data-dash="market-orders"]')?.addEventListener("click", (e) => {
    root.querySelectorAll(".dash-sidebar nav button").forEach(btn => btn.classList.toggle("active", btn === e.currentTarget));
    renderClientMarketOrders(root, actions);
  });
}

function getMarketTopbar(container) {
...
function renderPrivateProviderReview(content, providers) {
  ...
  const cards = providers.map(profile => {
    ...
    const filesHTML = files.map((file, index) => `<div><b>F${index + 1}</b><span>${escapeHTML(file.name)}</span><small>${escapeHTML(file.type || "Portfolio file")} · Content X admin only</small></div>`).join("");
    ...
    return `<article><header><span>${escapeHTML(profile.name.split(/\s+/).map(part => part[0]).join("").slice(0,2))}</span><div><h3>${escapeHTML(profile.name)}</h3><p>${escapeHTML(profile.role)} · ${escapeHTML(profile.headline)}</p></div><em class="moderation-status ${statusClass}">${escapeHTML(profile.status)}</em></header><div class="private-provider-meta"><span><strong>Services</strong>${escapeHTML(profile.services || profile.skills || profile.role)}</span><span><strong>Availability</strong>${escapeHTML(profile.availability || "Not provided")}</span><span><strong>Turnaround</strong>${escapeHTML(profile.turnaround || "To confirm")}</span><span><strong>Assignments</strong>${assignedCount}</span></div><p>${escapeHTML(profile.bio)}</p><div class="private-provider-contact"><span>✉ ${escapeHTML(profile.email)}</span><span>◉ ${escapeHTML(profile.phone)}</span><strong>Visible only to Content X administrators</strong></div><div class="provider-review-samples private-samples">${portfolioHTML}</div><div class="private-price-guide">${priceHTML}</div><div class="provider-review-actions"><button data-private-provider-decision="Private · Approved for matching" data-provider-id="${profile.id}">Approve for matching</button><button data-private-provider-decision="Private · Changes requested" data-provider-id="${profile.id}">Request changes</button><button data-private-provider-decision="Private · Declined" data-provider-id="${profile.id}">Decline</button></div></article>`;
  }).join("");
  content.innerHTML = `<div class="private-provider-admin-head"><div><p class="eyebrow"><span></span>Content X talent vault</p><h2>Private provider listings</h2><p>Services, contact details, pricing and portfolios are visible only to authorized Content X administrators. Approval makes a provider eligible for matching, never public.</p></div><span>⌾ ADMIN ONLY</span></div>${providers.length ? `<div class="provider-review-list">${cards}</div>` : '<div class="empty-state"><span>⌾</span><h3>No private provider listings yet</h3><p>New service listings and private portfolios will appear here.</p></div>'}`;
