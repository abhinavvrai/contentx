...
function finishAccountAccess(user, returningTo) {
  ...
  localStorage.setItem("cx_access", JSON.stringify({ email:user.email, name:user.name, paid:true, account:true }));
...
export async function renderAccountDashboard(root, actions) {
  ...
  try {
    ...
    root.innerHTML = `<header class="account-head"><a class="brand" href="#home"><span class="brand-mark">CX</span><span>Content X</span></a><nav><a href="#pricing">Pricing</a><button type="button" data-account-logout>Sign out</button></nav></header><main class="account-dashboard"><section class="account-welcome"><div><p class="eyebrow"><span></span>Client account</p><h1>Hello, ${escapeHTML(data.user.name.split(" ")[0])}.</h1><p>Manage paid orders, send the full project brief and upload footage or references.</p></div><a class="pill pill-hot" href="#home">Choose another package →</a></section><section class="dashboard-help-strip account-help-strip"><article><span>1</span><strong>Open project</strong><small>Every paid order gets its own private workspace.</small></article><article><span>2</span><strong>Upload files</strong><small>Add footage, logos and reference links after the brief.</small></article><article><span>3</span><strong>Review delivery</strong><small>Track versions, comments, approvals and downloads.</small></article></section><section class="account-order-section"><div class="account-section-title"><div><h2>Your projects</h2><p>Detailed briefs and files are collected after payment, so checkout stays quick.</p></div><span>${orders.length} order${orders.length === 1 ? "" : "s"}</span></div><div class="account-orders">${orders.length ? orders.map(orderCard).join("") : `<div class="account-empty"><span>◇</span><h3>No paid projects yet</h3><p>Choose a video or podcast package to start.</p><a class="pill pill-dark" href="#home">See pricing →</a></div>`}</div></section></main>`;
