...
function bindNotificationSettings(root) {
  ...
  panel.querySelector("[data-mark-all-notifications]")?.addEventListener("click", async event => {
    const button = event.currentTarget; button.disabled = true; status.textContent = "Marking notifications read…";
    try {
      const result = await api(NOTIFICATION_API, { method:"POST", headers:{ "Content-Type":"application/json" }, body:JSON.stringify({ action:"mark_all_read" }) });
      panel.querySelectorAll("[data-notification-item]").forEach(item => { item.querySelector("[data-mark-notification-read]")?.replaceWith(Object.assign(document.createElement("em"),{ textContent:"Read" })); });
      document.querySelectorAll("[data-notification-badge]").forEach(badge => { badge.hidden = true; });
      status.textContent = result.updated ? `${result.updated} notification${result.updated === 1 ? "" : "s"} marked read.` : "You are already caught up.";
    } catch (error) { status.textContent = error.message || "Could not update notifications."; }
